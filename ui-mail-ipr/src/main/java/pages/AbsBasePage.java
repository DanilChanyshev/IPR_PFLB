...
public abstract class AbsBasePage<T> extends PageActions {
  ...
  private static final String MOVE_TO_BIN_MESSAGE = "Перемещено в папку «Корзина»";
  private static final String DELETE_LETTER_MESSAGE = "Письмо удалено";
  ...
  protected T open() {
    driver.get(BASE_URL.concat(getPath()));
    return (T) this;
  }

  protected T open(String... data) {
    driver.get(BASE_URL.concat(getPathWithData(data)));
    return (T) this;
  }
  ...
  public T checkDeleteMessage() {
    Assertions.assertThat(getText(SNACK_BAR_MESSAGE))
            ...
            .isIn(MOVE_TO_BIN_MESSAGE, DELETE_LETTER_MESSAGE);
    return (T) this;
  }
